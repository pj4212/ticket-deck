...
function eventInfoBlock(event) {
  ...
  return `<tr><td style="padding:12px 40px;"><table width="100%" style="background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;padding:16px;"><tr><td><p style="margin:0;font-size:13px;color:#94a3b8;">Date</p><p style="margin:2px 0 10px;font-size:14px;color:#334155;font-weight:600;">${fmtDate(event.event_date)}</p><p style="margin:0;font-size:13px;color:#94a3b8;">Time</p><p style="margin:2px 0 10px;font-size:14px;color:#334155;font-weight:600;">${fmtTime(event.start_datetime)} – ${fmtTime(event.end_datetime)}${event.timezone ? ` (${event.timezone.split('/').pop()})` : ''}</p><p style="margin:0;font-size:13px;color:#94a3b8;">Format</p><p style="margin:2px 0 0;font-size:14px;color:#334155;font-weight:600;">${mode}</p></td></tr></table></td></tr>`;
}
...
function buildTicketConfirmation(brand, ticket, event, ttName, manageUrl, venue) {
  ...
  return wrap(brand, `
    ...
    <tr><td style="padding:28px 40px 12px;"><p style="margin:0;font-size:15px;color:#334155;">Hi <strong>${ticket.attendee_first_name}</strong>,</p><p style="margin:8px 0 0;font-size:14px;color:#64748b;line-height:1.6;"><strong>Type:</strong> ${ttName} (${modeLabel})<br><strong>Date:</strong> ${fmtDate(event.event_date)}<br><strong>Time:</strong> ${fmtTime(event.start_datetime)} – ${fmtTime(event.end_datetime)}${event.timezone ? ` (${event.timezone.split('/').pop()})` : ''}</p></td></tr>
    ${accessBlock}
    ${qrBlock}
