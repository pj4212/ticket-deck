...
import { Loader2, Mail, Search, RefreshCw, Send, RotateCcw, AlertTriangle, CheckCircle2, XCircle, Clock } from 'lucide-react';
...
export default function EmailManagement() {
  ...
  async function load() {
    ...
    const [emailLogs, evts] = await Promise.all([
      base44.entities.EmailLog.filter(filter),
      base44.entities.Event.filter(workspaceId ? { workspace_id: workspaceId, status: 'published' } : { status: 'published' }),
    ]);
    setLogs(emailLogs.sort((a, b) => (b.created_date || '').localeCompare(a.created_date || '')));
    setEvents(evts);
    setLoading(false);
  }
  ...
  const handleSendUpdate = async () => {
    if (!updateDialog || !updateMessage.trim()) return;
    setSending(true);
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => setUpdateDialog('select')}>
            <Send className="h-4 w-4 mr-1.5" />Send Event Update
          </Button>
      ...
      <Dialog open={!!updateDialog} onOpenChange={(o) => !o && setUpdateDialog(null)}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div>
              <Label>Message</Label>
              <Textarea value={updateMessage} onChange={e => setUpdateMessage(e.target.value)} placeholder="What's changed? This will be sent to all ticket holders." rows={4} />
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setUpdateDialog(null)}>Cancel</Button>
              <Button onClick={handleSendUpdate} disabled={sending || !updateMessage.trim() || updateDialog === 'select'}>
